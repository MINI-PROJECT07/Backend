...
const {
  sendNotificationToNearestHospital,
} = require("../util/notificationUtils.js");
...
const createAccident = async (req, res) => {
  try {
    ...
    if (!errors.isEmpty()) {
      ...
      return res
        .status(400)
        .json({ success: success, error: "", message: "Validation Error" });
    }
...
const getAccidentsNearestAuto = async (req, res) => {
  try {
    ...
    if (!hospital) {
      return res
        .status(400)
        .json({ success: false, error: "Hospital not found", message: "" });
    }
    const notifications = await NotificationH.find({
      hospital: hospitalId,
      status: "pending",
    });
    let nearestAccidents = [];
